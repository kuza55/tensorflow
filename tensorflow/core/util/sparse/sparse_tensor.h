...
template <typename T>
std::vector<SparseTensor> SparseTensor::Slice(const SparseTensor& input_tensor,
  ...
  typename TTypes<int64>::Matrix output_indices_t;
  typename TTypes<T>::Vec output_values_t;
  auto input_values_t = input_tensor.values().vec<T>();
  auto input_indices_t = input_tensor.indices().matrix<int64>();
